...
## 6. 角速度映射拟合结果 (yaw_total vs cmd_w)

通过 `niheyaw.py` 对不同线速度下的角速度响应进行线性回归拟合，得到如下结果：

$$\text{yaw\_total} = k \cdot \text{cmd\_w} + b$$

| 线速度 v (m/s) | 斜率 k | 截距 b | R² |
|:--------------:|:------:|:------:|:---:|
| 0.5 | 1.1512 | -0.0963 | 0.9993 |
| 1.0 | 0.9177 | -0.0577 | 0.9992 |
| 1.5 | 0.5567 | -0.0127 | 0.9976 |
| 2.0 | 0.4231 | -0.0112 | 0.9998 |
| 2.5 | 0.1796 | -0.0100 | 0.9470 |
| 3.0 | 0.1505 | -0.0135 | 0.9664 |
| 3.5 | 0.1196 | -0.0089 | 0.9954 |
| 4.0 | 0.1164 | -0.0131 | 0.9766 |

**规律**: 随线速度增大，角速度增益 k 显著下降（从 ~1.15 降至 ~0.12），说明高速下角速度响应灵敏度降低，控制时需相应放大角速度指令。