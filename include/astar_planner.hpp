...
public:
    ...
    // --- 新增：路径平滑算法 (Gradient Descent) ---
    // weight_data: 保持原始路径形状的权重
    // weight_smooth: 平滑力度
    ...
    static std::vector<Point> resample_path(const std::vector<Point>& raw_path, double step_size) {
        ...
        resampled.push_back(raw_path.front());
        double accumulated_dist = 0.0;
        for (size_t i = 0; i < raw_path.size() - 1; ++i) {
            double dx = raw_path[i+1].x - raw_path[i].x;
            double dy = raw_path[i+1].y - raw_path[i].y;
            double seg_len = std::hypot(dx, dy);
            if (seg_len < 1e-4) continue;
            double current_seg_dist = 0.0;
            double needed = step_size - accumulated_dist;
                            ...
                            const std::vector<Point>& all_points) 
    {
        ...
        return run_astar(start_gx, start_gy, goal_gx, goal_gy, width, height, occupancy_grid, min_gx, min_gy);
    }

private:
    Config cfg_;

    std::vector<Point> run_astar(int sx, int sy, int gx, int gy, int w, int h, const std::vector<int8_t>& grid, int min_gx, int min_gy) {
        ...
        int iter = 0;
        while(!open_set.empty() && iter++ < 25000) { 
